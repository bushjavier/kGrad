...
internal class ValueTest {
    ...
    @Test
    fun testOperationsRelu() {

        val x1 = Value(-4.0)
        val z1 = x1 * 2.0 + 2.0 + x1
        val q1 = z1.relu() + z1 * x1
        val h1 = (z1 * z1).relu()
        val y1 = h1 + q1 + q1 * x1
        y1.backward()
        ...
        NDManager.newBaseManager().use { manager ->
            ...
            Engine.getInstance().newGradientCollector().use { gc ->
                val z = x.mul(2.0).add(2.0).add(x)
                val q = Activation.relu(z).add(z.mul(x))
                val h = Activation.relu(z.mul(z))
                val y = h.add(q).add(q.mul(x))
                gc.backward(y)
    ...
    @Test
    fun testOperationsLeakyRelu() {

        val x1 = Value(-4.0)
        val z1 = x1 * 2.0 + 2.0 + x1
        val q1 = z1.leakyRelu() + z1 * x1
        val h1 = (z1 * z1).leakyRelu()
        val y1 = h1 + q1 + q1 * x1
        y1.backward()
        ...
        NDManager.newBaseManager().use { manager ->
            ...
            Engine.getInstance().newGradientCollector().use { gc ->
                val z = x.mul(2.0).add(2.0).add(x)
                val q = Activation.leakyRelu(z, 0.01f).add(z.mul(x))
                val h = Activation.leakyRelu(z.mul(z), 0.01f)
                val y = h.add(q).add(q.mul(x))
                gc.backward(y)
                ...
                //forward pass went well
                assert(abs(y.getDouble() - y1.data) < tolerance)

                //backward pass went well
                assert(abs(x.gradient.getDouble() - x1.grad) < tolerance)
            }
        }
    }

    @Test
    fun testMoreOperationsReluLeakyRelu() {
        ...
        c1 = c1 + c1 + 1.0
        c1 = c1 + c1 + 1.0 + (-a1)
        d1 = d1 + d1 * 2.0 + (b1 + a1).relu()
        d1 = d1 + d1 * 3.0 + (b1 - a1).leakyRelu()
        val e1 = c1 - d1
        val f1 = e1.pow(2.0)
        var g1 = f1 / 2.0
        g1 +=  f1 / 10.0
        ...
        NDManager.newBaseManager().use { manager ->
            ...
            Engine.getInstance().newGradientCollector().use { gc ->
                ...
                c = c.add(c.add(1.0))
                c = c.add(c.add(1.0)).add(a.neg())
                d = d.add(d.mul(2.0).add(Activation.relu(b.add(a))))
                d = d.add(d.mul(3.0).add(Activation.leakyRelu(b.sub(a), 0.01f)))
                val e = c.sub(d)
                val f = e.pow(2.0)
                var g = f.div(2.0)
                g = g.add(f.div(10.0))
